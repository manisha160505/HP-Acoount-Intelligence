...
def prompt_block(persona: dict) -> str:
    ...
    **The name is deliberately absent.** Everything here is role: title,
    ...
    for point in (persona.get("pain_points") or [])[:4]:
        lines.append("- A concern recorded against this role "
                     "[stakeholder_talking_points]: %s" % _text(point))

    for card in (persona.get("objections") or [])[:5]:
        lines.append("- An objection this role is expected to raise "
                     "[objection_reframe_cards]: %s" % card["objection"])
