...
# needs ONE resolvable citation for a whole answer, so a persona could tag one
# sentence and invent three around it - none carrying a digit, so the figure
# check never sees them. And it treats anything that is not a refusal as
# asserting facts, so a persona could not say "go on" without being rejected.
#
# On this branch citations are evidence ids resolved through the registry, so
# the overlap check is tighter than a whole-payload design allows: a sentence is
# compared against the source_text of the exact line it cited.
# ---------------------------------------------------------------------------

EVIDENCE = {
    "astra_obj#c1": "We are already using ManageEngine for device management.",
    "astra_stk#c2": "Irvan Nr is Chief Operating Officer, department Information Technology.",
    "astra_fin#c3": "Net Revenue FY2025: IDR 323,392 billion.",
}
PASSAGES = ["\n".join("%s [%s]" % (text, eid) for eid, text in EVIDENCE.items())]
BANNED = ["Stephen Dharma", "Stephen", "Dharma", "Irvan Nr", "Irvan"]


@pytest.fixture
def gate(monkeypatch):
    """`_validate_roleplay` with the evidence registry stubbed.

    `ev.resolve` is a scoped Mongo read in production - that scoping IS the
    after-generation account isolation - so it is replaced rather than widened
    here, and an id outside EVIDENCE comes back invalid exactly as one from
    ...
    monkeypatch.setattr(chat.ev, "resolve", fake_resolve)
    return lambda text: chat._validate_roleplay("acct", text, PASSAGES, BANNED)


def test_conversational_glue_needs_no_source(gate):
    """The case that fails outright under the advisor's validator.

    "Hmm. Go on." asserts nothing, cites nothing, and `_validate` rejects it as
    stating facts without evidence - three times, then an error. A persona that
    cannot say "go on" cannot hold a conversation.
    """
    ok, reason, cited, _ = gate("Hmm. Go on.")
    assert ok is True, reason
    assert cited == []


def test_a_question_needs_no_source(gate):
    """Asking is how this role finds out what is being proposed."""
    ok, reason, _, _ = gate("So what exactly are you proposing to me?")
    assert ok is True, reason


def test_a_grounded_line_passes(gate):
    ok, reason, cited, _ = gate(
        "We are already using ManageEngine for device management "
        "[astra_obj#c1]. What would switching actually gain us?")
    assert ok is True, reason
    assert [row["evidence_id"] for row in cited] == ["astra_obj#c1"]


def test_invented_operational_colour_is_rejected(gate):
    """The failure this gate exists for, and the likeliest one.

    A contract term carries no digits, so the figure check cannot see it, and it
    sits beside a properly cited sentence, so the answer-level citation check is
    satisfied. Only a per-sentence rule catches it.
    """
    ok, reason, _, _ = gate(
        "We are already using ManageEngine [astra_obj#c1]. And we are locked "
        "into a three-year agreement with the incumbent until then.")
    assert ok is False
    assert "no source" in reason


def test_naming_any_real_person_is_rejected(gate):
    """Not only the role being played - a colleague too."""
    ok, reason, _, _ = gate("That sits with Stephen Dharma, not me [astra_stk#c2].")
    assert ok is False
    assert "named a real person" in reason


def test_a_citation_that_does_not_support_the_sentence_is_rejected(gate):
    """A tag proves the model asserted an attribution, not that it is true."""
    ok, reason, _, _ = gate(
        "Our procurement freeze runs until the new fiscal year [astra_fin#c3].")
    assert ok is False
    assert "does not support" in reason


def test_roleplay_did_not_weaken_figure_grounding(gate):
    """Pinned explicitly so a refactor cannot quietly drop it."""
    ok, reason, _, _ = gate(
        "Our device spend fell 4271 percent last year [astra_fin#c3].")
    assert ok is False
    assert "4271" in reason


def test_an_evidence_id_that_does_not_resolve_is_rejected(gate):
    """The same isolation the advisor has: another account's id fails here."""
    ok, reason, _, _ = gate("We looked at that already [aOTHER_fake#c1].")
    assert ok is False
    assert "do not resolve" in reason


def test_a_speaker_label_is_stripped(gate):
    """The one place the name could reappear after being kept from the model."""
    ok, reason, _, cleaned = gate(
