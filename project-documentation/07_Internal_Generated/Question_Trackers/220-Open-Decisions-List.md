...
4. **Blank domains.** Public Bank and Westpac have no Company Domain. Question: confirm publicbankgroup.com and westpac.com.au. Default: use those. Affects: all datasets for those two accounts.
   **Status: Westpac RESOLVED** (`westpac.com.au`; NZ account keeps `westpac.co.nz`). **Public Bank RESOLVED 25 Sep** (DEC-055): `pbebank.com` per the domain audit sheet; applied in the split.
...
6. **pbebank.com.** Named Public Bank Bhd by Exa and Public Bank Lao by PredictLeads. Question: which company? Default: Public Bank Bhd; the PredictLeads rows are withheld. Affects: one account.
   **Status: RESOLVED 25 Sep** (DEC-055): the audit sheet marks the "Public Bank Lao Limited" PredictLeads row OK, so the PredictLeads rows are attached to Public Bank Bhd in the split. History: Public Bank Lao is a wholly-owned subsidiary of Public Bank Berhad, hence the shared domain. BridgeAI will try to supply Public Bank Berhad rows for the PredictLeads file. PredictLeads rows withheld until then; Exa rows (which name Public Bank Bhd) attached.
