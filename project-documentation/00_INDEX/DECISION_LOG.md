...
- **Status:** SUPERSEDED on 25 Sep by DEC-052: the domain-audit sheet now gives the canonical domain per account (the four overrides should be checked against it). The split's alias rewrites (CONFLICT I-03) are moot once the sheet is used. Applied in the split on 25 Sep: aliases now point toward the audit sheet's domains. · **Impact:** every domain join.
...
- **Status:** RESOLVED 25 Sep by the audit sheet (DEC-055): Public Bank = pbebank.com and the "Public Bank Lao Limited" PredictLeads row is accepted as the account ("domain is matching irrespective of their names"). Our A4 concern is on record. Applied in the split on 25 Sep: Public Bank = pbebank.com, PredictLeads rows attached. · **Impact:** one account's PredictLeads data.
...
- **Current status:** CURRENT; file received 25 Sep 12:30 IST (see DEC-055 for what it settles). Supersedes the interim "PredictLeads domain canonical + four overrides" reading of DEC-013 and every domain the split derived (Public Bank, aliases). · **Impact:** every join; the split reads this sheet as its domain source since the 25 Sep rerun (`_ACCOUNTS.csv` carries its columns as `audit_*`); dashboard account names come from Column B, not vendor names.
...
- Versus the 25 Sep split: three domains must change (Posco posco-inc.com → posco.com; Pilipinas Shell pilipinas.shell.com.ph → shell.com.ph; Public Bank publicbankgroup.com → pbebank.com). **Applied 25 Sep:** the split reads the sheet, all 219 domains match, and `_ACCOUNTS.csv` carries columns B–H as `audit_master_company`, `audit_master_domain`, `audit_predictleads_company`, `audit_predictleads_domain`, `audit_domain_check`, `audit_considerations`. Column B "Master Company" (e.g. "AUSTRALIA POST - AU") is the dashboard display name.

### DEC-056 · Case-study order: use case → industry → APJ/APAC → outcome → T0 before T2 — CLIENT DECISION
- **Source:** MAIN, Dhruvi 25 Sep 2026 09:36 UTC (reply to Yogesh's 09:10 UTC F12 re-ask): "This is correct :1. Same use case → 2. Same industry → 3. APJ/APAC region → 4. Stated outcome/metric → 5. HP.com (T0) before third-party (T2). Yes, industry first, followed by APJ/APAC region … Please infer the region from the case-study page/URL where possible … Region should not override the use-case match."
- **Status:** RESOLVED (D48). · **Impact:** case-study ranking. **Engineering gap:** our 25 Sep 14:30 email told the client region "cannot be inferred from the URLs" and APJ is not applied; seven pages carry an APJ locale (au-en, nz-en, sg-en, my-en, ph-en, in-en) and Mazenod College WA names its state, so step 3 should be applied from those.

### DEC-057 · Evidence-tier gate on proof applies (option A) — CLIENT DECISION, recorded by us
- **Source:** round-4 explainer `07_Internal_Generated/Client_Facing_Round4/clarifying_opens_4_OPEN_and_CLARIFICATIONS.docx` (default A); Yogesh 25 Sep 09:10 UTC "F11. Evidence-tier gate on proof. is resolved"; our 25 Sep 14:30 UTC email lists F11 as settled; the 26 Sep reply does not contest it. The client's own words choosing A are not in the thread.
- **Status:** RESOLVED (D47). · **Impact:** an HP offering is named and proof attached at the Opportunity tier only, on every feature including the Objection Playbook and Content Studio.

### DEC-058 · Answers of 26 Sep 03:26 UTC to our 25 Sep 14:30 UTC email §5 — CLIENT DECISIONS / CLIENT DATA
Source for all sub-items: MAIN, Dhruvi Patel, 26 Sep 2026 03:26 UTC (message 1a0dbbfe55d00cde), answers highlighted inline in the quoted §5.
- **DEC-058a · Contact file delivered (C49):** "Resolved: Apollo_all_contacts(1) uploaded in the google drive folder; as sahaj mentioned , if we can pls be flexible on this ->we are pulling data and will comeback with few more details if we can". 2,296 contacts, 192 of 220 accounts, 28 with none. Status: PARTIALLY RESOLVED (D8) — coverage top-up may follow. Supersedes DEC-050's blocker. How the file is mapped is INT-11.
- **DEC-058b · GCP:** "can you please elaborate on project ID and region? I think region part is already solved in one of the opens which I shared yesterday." Status: OPEN (D42). We owe a plain explanation of "project ID". No readable document answers the GCP region; she may be thinking of the F12 APJ/APAC case-study region (DEC-056). Confirm the deployment region separately unless the unread 25 Sep 12:02 UTC attachments name one.
- **DEC-058c · Technographic Map risk labels:** "in one of the opens I replied that you can use the risk logic which you shared, hence which are these labels ?" The logic (round 3 F7) is approved; the labels are the Low / Medium / High Risk the client asked us to retain on 23 Sep. Status: RESOLVED (D37); no product change. Supersedes the D37 part of DEC-054 "still open". E23-3 ("Contextual — no direct HP line") remains OPEN.
- **DEC-058d · Filings for ANZ Holdings NZ, CIMB Group Holdings, CIMB Niaga:** six document URLs supplied. Recorded as `04_Data_and_Source_Definitions/Filings/filings_client_supplement_2026-09-26.csv` (C50, row_id S26-1..6, filings 1.csv's columns); the split and fetch_filings.py read it after filings 1.csv. Status: RESOLVED (E25-4). CIMB Niaga's URL blocks scripts (AWS WAF); downloaded by hand 26 Sep and placed via Filings/pdfs/_manual/.
- **DEC-058e · Bank Mandiri vs BCA:** "When you look at the urls , those redirect to Bank Mandiri , hence consider Bank Mandiri". The two rows with company "PT Bank Mandiri (Persero) Tbk" and territory "PT BANK CENTRAL ASIA TBK - ID" are filed under PT_BANK_MANDIRI_PERSERO (FILINGS_CLIENT_RULINGS in split_account_data.py). Status: RESOLVED (E25-5). Both PDFs downloaded by hand 26 Sep (the site refuses scripts); sha256 matches the index; placed via Filings/pdfs/_manual/.
- **DEC-058f · Hiring for the remaining accounts:** "thank you for this flexibility". D22 stays PARTIALLY RESOLVED; no delivery date.
...
- **Status:** OPEN — the single biggest blocker; 25 Sep: "will give that", one file only (DEC-054e). **Superseded 26 Sep by DEC-058a:** file delivered, 192 of 220 accounts covered. · **Impact:** Stakeholder Map, Opportunity Map, Objection Playbook, Content Studio, Message Evaluator for all accounts.
...
- **Status:** OPEN; "escalated to sahaj" (25 Sep); project id and region unknown. 26 Sep (DEC-058b): client asks us to explain "project ID"; says region was answered in the 25 Sep opens (possibly the F12 case-study region, not GCP). · **Impact:** 220-account deployment.
...
| INT-06 | 220-account split corrections: Westpac → westpac.com.au derived from the Website column (Public Bank now pbebank.com per the audit sheet, DEC-055); three Ministries of Defence and two Westpacs disambiguated by country suffix; five alias rewrites toward the audit sheet's domains; Jabil SG / MUFG Bangkok get no news or intent rows (no entity field), but since the 25 Sep 22:00 re-run their PredictLeads rows are split by the new company_name column (C48), dual-entity rows going to both (128 rows); NSW Education intent taken from C33 in place of the main file's "Unavailable" row; news limited to the 12 months to the run date (Exa 2,929 of 3,726 rows kept, RSS 2,876 of 4,159) with Exa kept over RSS on a duplicate (DEC-054a); Astra datasets filled from the seed. | `220 account split csv/_CORRECTIONS.txt` | Not sent as such; domain items now match DEC-052/055 (CONFLICT I-02/I-03 closed 25 Sep) |
...
| INT-11 | Apollo contact file (C49) → `prospect_contacts`, 26 Sep: rows matched on Company Name + Country Code = Sales Territory Name (2,275), else bare name with matching master country (21, Bunnings); domain cross-checked, no disagreements. Explorium column layout plus every Apollo field under `apollo_*` (the pilot's shape, which the Stakeholder Map already reads). **Not** filled because the file does not state them: buying-committee persona (Requested Role is what we asked the vendor to find, kept as `apollo_requested_role`), email status "valid" (only Email Confidence "provided"; the verification columns are empty), contact location (Country Code is the company's). `review_flags` on every row: 6 work emails that name a different person are **withheld** from the display columns (kept in `apollo_verified_work_email_as_delivered`, a column no extractor reads); 54 email domains that are not the company's, 1 team name ("Internal Channels"), 1 "Not matched", 5 phone look-ups needing review are flagged and kept. 130 rows are the same person under both sister entities (Jabil MY/SG, MUFG JP/Bangkok, UOB SG/MY) and go to both, as delivered, flagged. **Astra keeps its 23-contact pilot seed** (SEED_PREFERRED): the new file's 13 Astra contacts share none of the 23 names, so they are kept in `reference/apollo_prospect_contacts.csv` pending Yogesh's decision. | `split_account_data.py` (assign_apollo_contacts, SEED_PREFERRED); `_CORRECTIONS.txt` | Not sent |
