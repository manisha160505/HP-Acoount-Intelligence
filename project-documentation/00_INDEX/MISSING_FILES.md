...
| — | clarifying_opens_3_CLARIFICATIONS.docx and clarifying_opens_3_UNRESOLVED_v2.docx (**client-answered** copies) | Dhruvi, 25 Sep 2026 12:02 UTC, thread 1a082bca2581fea0 | Answers to C1 (Agribank / VPBank filings), the risk-logic approval and the GCP region the 26 Sep reply refers to. The same-named files in `05_Questions_and_Clarifications/Source_Documents/` are our unanswered versions. | `01_Client_Provided/Client_Answers/` |
...
Contact file (delivered 26 Sep, C49 — 28 accounts still without contacts; "few more details" may follow); Company Name + Country columns populated on every PredictLeads row (delivered 25 Sep, C48); Public Bank Bhd rows for PredictLeads; re-crawled Exa event dates; job openings for the 45 accounts without them; corrected filings rows for Fletcher / Fonterra / Astra (FIF); filings for Agribank and VPBank (or confirmation they belong on the exclusion list); GCP project id / region (26 Sep: client asks what "project ID" means); written feedback on HP-Account-Intelligence-Rules.docx (promised 10 Sep). See `06_Unresolved_and_Open/`.
