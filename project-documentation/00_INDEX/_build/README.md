...
To record a newly retrieved file (e.g. the Rulebook FINAL): set its `src=` in `build_docs.py`, delete the `.MISSING.md` placeholder, run `python3 build_docs.py` from the repo root, then run `python3 build_index.py` to regenerate `MASTER_DOCUMENT_INDEX.md` from `provenance_manifest.json`.
